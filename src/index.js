...
import { 
    ...
    deleteDateTime, createEvents, searchEvents
} from './app/app';
...
if (indexPage) {
    ...
    signupForm.addEventListener('submit', (e) => {
        ...
        let confirm = signupForm.confirm.value
        let dob = new Date(signupForm.dob.value)
        let today = new Date();
        today.setFullYear(today.getFullYear() - 18);
        let m = document.getElementById('signupMessage');
        ...
        if (password != confirm) {
            ...
            m.innerHTML = "Password mismatch."
        }

        if (dob > today) {
            valid = false
            m.classList.add("text-danger")
            m.innerHTML = "You must be 18 years or older."
        }
    
        if (valid) {
            createUserWithEmailAndPassword(auth, email.toLowerCase(), password).then((cred) => {
                // create a user doc record
                let fName = signupForm.firstName.value.toLowerCase()
                let lName = signupForm.lastName.value.toLowerCase()
                ...
                setDoc(doc(db,"users",cred.user.uid), docData)
                    .then(() => {
        ...
        Notification.requestPermission().then((permission) => {
            if (permission === 'granted') {
                ...
                signInWithEmailAndPassword(auth, email, password).then((cred) => {
                    ...
                    updateDoc(doc(db,"users",cred.user.uid),{deviceToken: messageToken}).then(() => {
                ...
                // ADD ALERT REQUEST PERMISSION TO BE TURN ON
                alert('Enhance your experience by enabling push notifications in your brower');
...
if (mainPage) {
    ...
    async function getEvents() {
        ...
        return await getDocs(eventsColl).then((snapshot) => {    
    ...
    if (eventsDiv) { 
        ...
        createEvents(eventsDiv, events, userData)
    }
...
if (eventPage) {
    ...
    // EVENT CREATE FORM
    let dateTimeDiv = document.getElementById('dateTimesDiv')
            ...
            if (userData.id == event.userId) {
                ...
                if (event.dateTimes.length > 0) {
                    ...
                    event.dateTimes.forEach((dt) => {
                        ...
                        dateTimeDiv.appendChild(dateTimeRow)
